...
/**
 * Creates demo tables and seeds sample data in the {@code demo} database of
 * the embedded MariaDB server used by {@link MySqlDatabaseProvider}, plus the
 * {@code filtered} shadow database backing the global dashboard filters. Runs
...
final class DemoDataInitializer {
    ...
    static synchronized void initialize(DataSource dataSource) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            // Standard double-quoted identifiers (e.g. "MONTH") in the DDL
            stmt.execute(
    ...
    /**
     * Creates the {@code filtered} shadow database. For each table the global
     * dashboard filters apply to it contains a filtering view under exactly
     * the same name as the base table; the views read connection-scoped
     * session user variables ({@code @from_date}, {@code @to_date},
     * {@code @regions}) through small stored functions (MySQL views cannot
     * reference user variables directly), and an unset variable is NULL, i.e.
     * "no filter". MySQL has no schema search path — a connection has exactly
     * one default database — so the shadow database also gets an unfiltered
     * passthrough view for every other demo table. With the connection's
     * default database set to {@code filtered}, unqualified table names then
     * resolve for all tables. All statements are idempotent.
     */
    private static void createFilteredSchema(Statement stmt)
            throws SQLException {
        stmt.execute("CREATE DATABASE IF NOT EXISTS filtered");

        stmt.execute("DROP FUNCTION IF EXISTS filtered.filter_from_date");
        stmt.execute("""
                CREATE FUNCTION filtered.filter_from_date() RETURNS DATE
                NOT DETERMINISTIC NO SQL
                RETURN CAST(@from_date AS DATE)
                """);

        stmt.execute("DROP FUNCTION IF EXISTS filtered.filter_to_date");
        stmt.execute("""
                CREATE FUNCTION filtered.filter_to_date() RETURNS DATE
                NOT DETERMINISTIC NO SQL
                RETURN CAST(@to_date AS DATE)
                """);

        // Returns a JSON array of region names, e.g. ["North","South"]
        stmt.execute("DROP FUNCTION IF EXISTS filtered.filter_regions");
        stmt.execute("""
                CREATE FUNCTION filtered.filter_regions() RETURNS TEXT
                NOT DETERMINISTIC NO SQL
                RETURN @regions
                """);
        ...
        stmt.execute("""
                ...
                WHERE (filtered.filter_from_date() IS NULL OR sale_date >= filtered.filter_from_date())
                  ...
                  AND (filtered.filter_regions() IS NULL
                       OR JSON_CONTAINS(filtered.filter_regions(), JSON_QUOTE(region)))
                """);

        stmt.execute("""
                CREATE OR REPLACE VIEW filtered.stock_prices AS
                SELECT * FROM demo.stock_prices
        ...
        stmt.execute("""
                ...
                SELECT * FROM demo.project_tasks
        ...
        // Unfiltered tables are reachable through passthrough views
        for (var table : List.of("employees", "products", "org_chart",
                "energy_flow", "website_traffic", "budget_items",
                "sales_pipeline", "kpi_metrics", "expense_categories")) {
            stmt.execute("CREATE OR REPLACE VIEW filtered." + table
                    + " AS SELECT * FROM demo." + table);
        }
    }
